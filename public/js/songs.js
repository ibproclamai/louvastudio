  ...
  container.innerHTML = filtered.map(s => {
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        ${s.video_url ? '<a href="' + escapeHtml(s.video_url) + '" target="_blank">Video</a>' : ''}
        ${s.observacoes ? '<br><small>' + escapeHtml(s.observacoes) + '</small>' : ''}
        ${vsList.length > 0 ? `<details style="margin-top:0.5rem"><summary><small>${vsList.length} trilha(s) VS</small></summary>${vsList.map(v => `<div style="margin-top:0.4rem;padding:0.4rem;background:var(--bg);border-radius:4px"><small><b>${escapeHtml(v.nome)}</b> <span class="badge badge-${escapeHtml(v.tipo)}">${escapeHtml(v.tipo)}</span><br><audio controls preload="none" src="${escapeHtml(v.url)}" style="width:100%;margin-top:0.3rem"></audio></small></div>`).join('')}</details>` : ''}
      </div>
      ${isAdmin ? `
